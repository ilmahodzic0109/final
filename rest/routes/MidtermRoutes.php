...
Flight::route('GET /midterm/investor_email/@email', function($email){
    ...
    */
    $dao = new MidtermDao();
    $result = $dao->investor_email($email);

    if ($result['status'] == 'invalid_format') {
        Flight::json(['message' => 'Invalid email format']);
    } elseif ($result['status'] == 'exists') {
        Flight::json(['message' => 'Investor ' . $result['first_name'] . ' ' . $result['last_name'] . ' uses this email address']);
    } elseif ($result['status'] == 'not_exists') {
        Flight::json(['message' => 'Investor with this email does not exist in database']);
    }
});
